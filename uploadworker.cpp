...
void UploadWorker::doWork(QFileInfoList files,
    ...
    for (i=0; i<files.size() && !QThread::currentThread()->isInterruptionRequested(); i++)
    {
        ...
        std::cout << std::endl;
        qiniuReturn qReturn;
        qReturn = qiniu.uploadFile(bucket.toStdString(),UTF8_To_string(remoteName.toStdString()),UTF8_To_string(localName.toStdString()),location);
...
std::string UploadWorker::string_To_UTF8(const std::string & str)
{
